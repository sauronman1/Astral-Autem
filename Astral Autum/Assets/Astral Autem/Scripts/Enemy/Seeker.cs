...
namespace FG {
	public class Seeker : MonoBehaviour
	{
		...
		[SerializeField] private float         _shooterCooldown;
		[SerializeField] private float         _distanceBeforeAiming;
		...
		private void Update()
		{
			...
			if (_timeSinceLastShot > _shooterCooldown)
			{
				Shoot();
		...
		private void Move()
		{
			...
			if (Vector2.Distance(_transform.position, _endPos) < _distanceBeforeAiming)
			{
				if (_player != null)
				{
